...
if (( "$GIT_STATUS" > 0 )); then
	git checkout master
	git pull
	cd eFormAPI/Plugins/Inventory.Pn/Inventory.Pn
  CURRENT_NUMBER_OF_COMMITS=`git log --oneline | wc -l`

	PACKAGES=('Microting.eForm' 'Microting.eFormApi.BasePn' 'Microting.eFormInventoryBase' 'Magick.NET-Q16-x64')
	PROJECT_NAME='Inventory.Pn.csproj'
