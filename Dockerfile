...
COPY eform-angular-inventory-plugin/eFormAPI/Plugins/Inventory.Pn ./Inventory.Pn
RUN dotnet publish eFormAPI.Web -o eFormAPI.Web/out /p:Version=$GITVERSION --runtime linux-x64 --configuration Release
RUN dotnet publish Inventory.Pn -o Inventory.Pn/out /p:Version=$PLUGINVERSION --runtime linux-x64 --configuration Release

# Build runtime image
FROM mcr.microsoft.com/dotnet/aspnet:7.0
WORKDIR /app
COPY --from=build-env /app/eFormAPI.Web/out .
RUN mkdir -p ./Plugins/Inventory.Pn
COPY --from=build-env /app/Inventory.Pn/out ./Plugins/Inventory.Pn
