...
def main():
    ...
    if int(get_git_status()) > 0:
        ...
        os.chdir("eFormAPI/Plugins/Inventory.Pn/Inventory.Pn")

        current_number_of_commits = get_commit_count()
        packages = [
            'Microting.eForm', 'Microting.eFormApi.BasePn', 'Microting.eFormInventoryBase', 'Magick.NET-Q16-x64'
        ]
        process_repository('Inventory.Pn.csproj', packages, 'eform-angular-items-planning-plugin')
